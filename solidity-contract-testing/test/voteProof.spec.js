...
contract('VoteProofVerifier.sol', () => {
  ...
  for (let i = 0; i < 10; i++) {
    ...
    for (const testCase of testCases) {
      it(`ZKP Verification for (p: ${testCase[0]}, g: ${
        testCase[1]
      })`, async () => {
        let pk;
        try {
          pk = FFelGamal.Encryption.generateKeysZKP(
            testCase[0],
            testCase[1]
          )[0];
        } catch (error) {
        ...
        const yesProof = FFelGamal.VoteZKP.generateYesProof(
          yesEnc,
          pk,
          uniqueID
        );
        ...
        assert.isTrue(
          yesVoteVerified,
          'YES-Proof could not be verified by the contract'
        );
        ...
        assert.isTrue(
          noVoteVerified,
          'NO-Proof could not be verified by the contract'
        );
      });
    }
  }
