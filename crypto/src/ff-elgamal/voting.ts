...
export const generateYesVote = (sp: SystemParameters, pk: BN): Cipher => Encryption.encrypt(1, sp, pk)
export const generateNoVote = (sp: SystemParameters, pk: BN): Cipher => Encryption.encrypt(0, sp, pk)
...
export const addVotes = (votes: Cipher[], sp: SystemParameters): Cipher => {
  return votes.reduce((previous, current) => Encryption.add(previous, current, sp), generateBaseVote())
}
