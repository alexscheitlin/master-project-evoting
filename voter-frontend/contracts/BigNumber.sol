...
library BigNumber {
    ...
    /** @dev prepare_add: Initially prepare bignum instances for addition operation; internally calls actual addition/
      * subtraction, depending on inputs. In order to do correct addition or subtraction we have to handle the sign.
    ...
    /** @dev bn_add: takes two instance values and the bitlen of the max value, and adds them.
      ...
      * in any order of size, and of different signs (handled in prepare_add). As values may be of different size,
      * inputs are considered starting from the least significant words, working back.
      * The function calculates the new bitlen (basically if bitlens are the same for max and min, max_bitlen++) and
   ...
   function bn_sub(bytes memory max, bytes memory min) private pure returns (bytes memory, uint) {
    ...
    function bn_mul(instance memory a, instance memory b) internal view returns(instance memory res){
        // add_and_square = (a+b)^2
        res = op_and_square(a,b,0);

        //no need to do subtraction part of the equation if a == b; if so, it has no effect on final result.
        if(cmp(a,b,true)!=0){

            // sub_and_square = (a-b)^2
            instance memory sub_and_square = op_and_square(a,b,1);

            // res = add_and_square - sub_and_square
            res = prepare_sub(res,sub_and_square);
        }

        // res = res / 4
        res = right_shift(res, 2);
     }


    /** @dev op_and_square: takes two instances, performs operation 'op' on them, and squares the result.
      * bn_mul uses the multiplication by squaring method, ie. a*b == ((a+b)^2 - (a-b)^2)/4. using modular
      * exponentation precompile for squaring. this requires taking a special modulus value of the form:
    ...
    /** @dev prepare_modexp: takes base, exponent, and modulus, internally computes base^exponent % modulus,
      * and creates new instance. this function is overloaded: it assumes the exponent is positive. if not,
      * the other method is used, whereby the inverse of the base is also passed.
      *
    ...
    /** @dev prepare_modexp: takes base, base inverse, exponent, and modulus, asserts inverse(base)==base inverse,
     ...
     function prepare_modexp(instance memory base, instance memory base_inverse, instance memory exponent, instance memory modulus) internal view returns(instance memory result) {
        ...
        require(cmp(base_inverse, mod_inverse(base,modulus,base_inverse), true)==0);
    ...
    function modadd(instance memory a, instance memory b, instance memory modulus) internal view returns(instance memory res) {
        res = bn_mod(prepare_add(a,b), modulus);
    }
