...
import { Cipher, Summary } from '../lib/crypto/models';
import { EccElGamal, EccElGamalVoting } from '../lib';
...
const EccElGamalComponent: React.FC = () => {
  ...
  return (
    <div>
      ...
      <br></br>
      <br></br>
      Public Key: {JSON.parse(JSON.stringify(publicKey))[0]}{' '}
      {JSON.parse(JSON.stringify(publicKey))[1]}
      <br></br>
      ...
      {votes.map((vote, i) => (
        <div key={i}>
          {JSON.stringify(vote.c1)}, {JSON.stringify(vote.c2)} -{' '}
          {JSON.stringify(EccElGamal.decrypt(vote, privateKey))}
        </div>
      ))}
    </div>
  );
